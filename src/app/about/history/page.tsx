...
export default function HistoryPage() {
  ...
  return (
    <main className="min-h-screen bg-[#fdfbf7] text-gray-800">
      ...
      {/* --- HERO SECTION: Vintage Vignette --- */}
      <section className="relative h-[60vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        {/* Background Image with warm overlay */}
        <div className="absolute inset-0 z-0">
          <div className="absolute inset-0 bg-linear-to-b from-red-900/60 to-amber-900/70 z-10 mix-blend-multiply"></div>
          <div className="absolute inset-0 bg-black/20 z-10"></div>
          <Image
            src="https://images.unsplash.com/photo-1495562569060-2eec283d3391?w=1600&q=80"
        ...
        <div className="container mx-auto px-4 relative z-20 text-center">
          <div className="inline-flex items-center justify-center p-4 bg-white/10 backdrop-blur-md rounded-full border border-white/20 mb-6 shadow-xl">
            <Feather className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-serif font-bold mb-4 text-white drop-shadow-lg tracking-wide">
        ...
        {/* FIXED: Smooth Curved Edge Effect at Bottom */}
        <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-0">
           <svg className="relative block w-full h-[60px] sm:h-[100px]" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
              {/* This new path creates the smooth convex curve */}
              <path d="M0,0V120H1200V0Q600,120 0,0Z" className="fill-[#fdfbf7]"></path>
          </svg>
        </div>
      </section>
            ...
            {milestones.map((milestone, index) => (
              <div key={index} className={`relative flex flex-col md:flex-row gap-12 items-center ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className="w-full md:w-1/2 pl-12 md:pl-0">
                  <div className={`p-6 rounded-xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] border border-gray-100 bg-white transition-all duration-500 relative group ${
                    clickedIndex === index ? 'scale-105 shadow-[0_8px_30px_-4px_rgba(167,27,38,0.3)] border-red-200' : 'hover:-translate-y-1'
                  }`}>
                ...
                >
                  <div className={`w-3 h-3 rounded-full bg-red-700 transition-all duration-300 ${
                    clickedIndex === index ? 'animate-ping' : 'group-hover/circle:scale-125'
                  }`}></div>
                  {clickedIndex !== index && (
