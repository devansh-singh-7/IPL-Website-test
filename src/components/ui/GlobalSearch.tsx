...
const GlobalSearchInner: React.FC<GlobalSearchProps> = ({
  ...
  const handleSelect = useCallback((href: string, title: string) => {
    // Navigate directly to suggestion reference without forcing search param unless it's a news-events search context
    if (onSelect) onSelect(href, title); else router.push(href)
    setQuery(title)
    setOpen(false)
    setActiveIndex(-1)
    // If suggestion points to news-events without explicit highlight, apply query param for filtering
    if (href.startsWith('/news-events') && !href.includes('highlight=')) {
      router.replace(`/news-events?q=${encodeURIComponent(title)}`)
    }
  }, [onSelect, router])
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      />
      {open && (
        <ul className={`absolute left-0 right-0 ${variant === 'mobile' ? 'mt-3' : 'mt-2'} bg-white border border-neutral-200 rounded-xl shadow-lg overflow-hidden z-50 text-sm animate-fade-in`} role="listbox">
